    ...
    for line in voteData:
        ...
        if len(data) == 5:
            ...
            if stateYear not in importedStatesYear:
                ...
                sId = stateFPs[sName]
                # import State Data
                ins = metadata.tables['States'].insert().values(StateId=sId, StateName=sName, Year=int(year),
                                                                ClickCount=0)
                conn.execute(ins)
    ...
    for line in geoData:
        ...
        for yr in range(year, year+10):
            ...
            s = "SELECT "\
                ...
                + " States.Year = " + str(yr) + " and States.StateId = "\
                + str(sId)\
                + " and Districts.DistrictId = "\
                + str(dId)
...
def importDistrictsAndVote(path):
    ...
    for line in districtData:
        ...
        if rVote == "Unopposed": # Check if a party was unopposed if it is make -1 a FLAG that will mean unopposed
            rVote = -1
            dVote =  0
            ...
            dVote = -1
        # get fk using sName
        s = "SELECT States.Id FROM gerrymandering.States WHERE States.StateName = \'" + sName + "\' and \
        States.Year = " + year
        for row in conn.execute(s):
            sFK = row[0]
        ...
        # insert Vote data
        ins = metadata.tables['Votes'].insert().values(DistrictId = districtPK[0], Party = "Republican",
                                                       voteCount=rVote)
        conn.execute(ins)
